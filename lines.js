function humanRightsScorePerYear() {
  ...
  d3.csv(path).then(function(rawData) {
    rawData = d3.nest()
              ...
              .entries(rawData);
    const countries = ["Iceland", "Luxembourg", "France", "Norway", "India", "North Korea", "Taiwan", "United States", "Pakistan", "Ireland", "China", "Japan", "Russia"];
    rawData = rawData.filter(el => countries.includes(el.key))
    drawLines(rawData, "Human rights score in each country per year");
  })
}
...
function drawLines(rawData, title) {
  ...
  rawData[0].values.forEach(function(value){
    data['dates'].push(new Date(value.key, 1, 1));
  });
